 ...
 * ============================================================ */
var CACHE_VERSION = 'nakoda-mis-v325';  /* v325: offline attendance punches that were stuck on a phone for ever now send by themselves. A punch saved mid-attempt kept a 'hold' flag that was never cleared, so the background queue skipped it on every cycle — and a held check-in blocked its own check-out. The queue also kept the OLDEST eight punches, so once eight were stuck, today's punch was discarded the moment it was saved. Bumping this version is what pushes the fixed attendance.js onto the phones that are carrying a backlog. Includes all of v324. */  /* v324: the public QR membership page. card.html and qr-poster.html are deliberately NOT part of the app shell and are excluded from this worker entirely (see the fetch handler) — a customer scanning a standee must always get the live page, and a staff phone must never serve them a cached one. Includes all of v323. */  /* v323: Outsource sits beside Collect sample — on the dashboard Quick log and on the Sample collection page, in the same order in both. Includes all of v322. */  /* v322: the outsource board fixes — rows folded into Collected are DRAWN under Collected instead of silently disappearing, the panel stops calling an outsource a walk-in, the send popup names the lab, and the quick-log tile stops eating the call outcome. Includes all of v321. */  /* v321: process 3 — outsource to report delivery. Two fields, two stages, and the desk holds one task until the outside lab's report goes out. Includes all of v320. */  /* v320: the Result and Verified stages are removed — a collected sample goes straight to the report desk. Three stages: ordered, collected, sent. Includes all of v319. */  /* v319: Order to delivery books a 5-minute slot at the branch counter — capacity is the number of chairs, not one, so a three-chair branch never reads full with two empty. Includes all of v318. */  /* v318: Sample collection opens the phlebotomist's diary — his month calendar with free days in green, his morning and evening times, click one and the blood-collection task is his. Includes all of v317. */  /* v317: home-visit orders are booked into 30-minute slots instead of a typed time — the grid shows what is free, a booked box is gone, and the order is finally stamped on the day that was chosen rather than always today. Repeat patients are found by name or number and bring their address with them. Includes all of v316. */  /* v316: bulk WhatsApp membership-card send. Includes all of v315. */  /* v315: arrived-in-lab trend on the main dashboard,
...
self.addEventListener('fetch', function(e){
  ...
  /* v324 — PUBLIC PAGES ARE NOT OURS TO CACHE.
     card.html is the page behind the printed QR standee and qr-poster.html prints those standees.
     Neither is in SHELL, so without this they would be cached on first visit as a "genuine miss"
     and a staff phone that once opened a customer link would keep serving that copy for the life of
     the cache. Returning here leaves them to the network, exactly like any ordinary web page. */
  if (/\/(card|qr-poster)\.html$/.test(url.pathname)) return;
  e.respondWith(
