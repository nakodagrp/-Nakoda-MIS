...
(function(){
  ...
  function openVideoForm(v,secs){ v=v||{}; VQ=(v.questions&&v.questions.length)?v.questions.map(function(q){return {q:q.q,options:(q.options||['','','','']).slice(),answer:Number(q.answer)||0};}):[];
    ...
    var body='<div class="grid2"><div class="field full"><label>Title</label><input id="vTitle" class="in" value="'+esc(v.title||'')+'"></div>'+
      ...
      '<div class="field full"><label style="display:flex;align-items:center;gap:8px">Visible to roles <span id="vRolesAll" style="margin-left:auto;cursor:pointer;font-size:12px;font-weight:500;background:#e6f1fb;color:#185fa5;border:1px solid #85b7eb;border-radius:16px;padding:4px 12px">Select all</span></label><div class="rolechips" id="vRoles">'+roles.map(function(r){var n=r.Role||r;return '<span class="rc'+(vr.indexOf(n)>=0?' on':'')+'" data-r="'+esc(n)+'">'+esc(n)+'</span>';}).join('')+'</div></div>'+
    ...
    document.querySelectorAll('#vRoles .rc').forEach(function(c){ c.onclick=function(){ c.classList.toggle('on'); syncRolesAll(); }; });
    function syncRolesAll(){ var b=$id('vRolesAll'); if(!b) return; var chips=[].slice.call(document.querySelectorAll('#vRoles .rc')); var allOn=chips.length&&chips.every(function(c){return c.classList.contains('on');}); b.textContent=allOn?'Clear all':'Select all'; }
    (function(){ var b=$id('vRolesAll'); if(b){ b.onclick=function(){ var chips=[].slice.call(document.querySelectorAll('#vRoles .rc')); var allOn=chips.length&&chips.every(function(c){return c.classList.contains('on');}); chips.forEach(function(c){ c.classList.toggle('on',!allOn); }); syncRolesAll(); }; syncRolesAll(); } })();
