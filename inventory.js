...
(function(){
  ...
  function openItemForm(it){ it=it||{};
    var body='<div class="grid2"><div class="field full"><label>Item name</label><input id="itName" class="in" value="'+esc(it.name||'')+'"></div>'+
      '<div class="field"><label>Category</label><select id="itCat" class="in">'+['Consumable','Reagent','Kit','Equipment','Other'].map(function(c){return '<option'+(c===it.category?' selected':'')+'>'+c+'</option>';}).join('')+'</select></div>'+
      ...
      '<div class="field"><label>Default vendor</label><select id="itVen" class="in"><option value="">—</option>'+INV.vendors.map(function(v){return '<option value="'+esc(v.vendorId)+'"'+(v.vendorId===it.vendorId?' selected':'')+'>'+esc(v.name)+'</option>';}).join('')+'</select></div>'+
