...
(function(){
  ...
  function purposeBadge(p){
    if(p==='membership_card') return '<span class="badge" style="background:#185fa522;color:#185fa5">Membership card</span>';
    ...
    box.innerHTML='<div class="table-wrap"><table><thead><tr><th>Template</th><th>Lang</th><th>Header</th><th>Params</th><th>Use for</th><th>Status</th><th></th><th></th></tr></thead><tbody>'+
      TPLS.map(function(t){
        return '<tr>'+
          ...
          '<td>'+purposeBadge(String(t.purpose||'general'))+'</td>'+
  ...
  function openTplEditor(tplId){
    ...
    var body='<div class="grid2">'+
      ...
      '<div class="field full"><label>Use for</label><select id="wt_purpose">'+puOpts+'</select>'+
        '<div style="font-size:11px;color:#999;margin-top:4px">Only one template can be "Membership card" at a time — it becomes the default for Option 3 on all branches.</div></div>'+
    ...
    document.getElementById('wt_save').onclick=function(){
      var data={ tplId:tplId||'', name:val('wt_name').toLowerCase().trim(), language:val('wt_lang'),
        ...
        purpose:document.getElementById('wt_purpose').value, paramHints:document.getElementById('wt_hints').value,
