 ...
 * ============================================================ */
var CACHE_VERSION = 'nakoda-mis-v401';  /* v401: (1) Cancelled campaigns are now unconditionally excluded from Campaign History everywhere (apiMsgListCampaigns) -- reported twice, so no more per-row Delete click needed, they just dont come back once this backend redeploys. (2) The App vXXX / cache vYYY build-check + Check update button used to live ONLY inside the mobile ⋯ More drawer, which is display:none above 760px -- meaning it was literally unreachable on the desktop app, which is why deploy status could never be confirmed there. Now also in the desktop topbar (index.html), always visible -- push index.html this time too, not just the bundle/sw. */
