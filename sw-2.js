 ...
 * ============================================================ */
var CACHE_VERSION = 'nakoda-mis-v400';  /* v400: SAFETY FIX -- membership-card templates (WA_Templates.purpose 'membership_card'/'membership_benefits') can no longer be picked in Bulk Message Send or Timely Message, on either side (picker AND save, so a stale page cant bypass it) -- this is the actual root cause of the card/text mixup: those templates image header and {{1}}/{{2}} mean ONE patient own card, but Bulk/Timely always used ONE shared image + branch/lead-name for the whole campaign. Send card templates from Membership Cards instead. Also: any OTHER media-header template in Bulk Message Send now shows a loud warning that its file sends identically to everyone, not per patient. */
