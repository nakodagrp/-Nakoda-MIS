...
(function(){
  ...
  function loadExpenses(){ API.listLedger(ACC.branch,ACC.ym,false).then(function(r){ var box=$id('accBody'); if(!box) return; if(!r||!r.ok){ box.innerHTML='<div class="empty">'+esc((r&&r.error)||'')+'</div>'; return; }
    ...
    function ledgerAction(ledId,act,reason){ return API.setLedger(ledId,act,reason||''); }
  ...
  function openExpenseForm(){
    ...
    /* v308: the id is generated HERE, once, and reused if the request has to be retried. Without it a
       lost reply meant the outbox replayed the expense and the server appended a second row under a new
       id — the four identical UDHANA rows. Generated outside the handler so a retry cannot re-roll it. */
    var _exLedId=API.newLedId();
    $id('exSave').onclick=function(){ var amt=$id('exAmt').value; if(!amt){ $id('exMsg').innerHTML='<div class="msg error">Amount required.</div>'; return; } this.disabled=true;
      API.addLedger({ledId:_exLedId,branchId:ACC.branch,type:$id('exType').value,category:$id('exCat').value,amount:amt,mode:$id('exMode').value,party:$id('exParty').value,date:$id('exDate').value,billDate:$id('exBillDate').value,ifsc:$id('exIfsc').value,acct:$id('exAcct').value,bill:st.bill,billMime:st.billMime,qr:st.qr,qrMime:st.qrMime,note:$id('exNote').value}).then(function(r){ if(r&&(r.ok||r.offline)){ closeModal(); toast(r.offline?'Saved on device — will sync':'Sent to the accountant for approval'); loadExpenses(); } else { $id('exMsg').innerHTML='<div class="msg error">'+esc((r&&r.error)||'Failed')+'</div>'; var _b=$id('exSave'); if(_b) _b.disabled=false; } }); };
  }
